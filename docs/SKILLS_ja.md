...
AI が `run_skill_workflow` 経由でスキルワークフローを呼び出した場合、**`__` で始まらない変数はすべて自動的にチャット AI にツール結果として返されます**。値を「出力」するために末尾に `command` ノードを追加する必要はありません。チャット AI に見せたい値は単に `saveTo:` で保存するだけで OK です。ワークフロー内部だけで使いたい変数は `__` プレフィックスを付けてください。
...
チャット中にスキルワークフローが失敗すると、失敗したツール呼び出しに **ワークフローを開く** ボタンが表示されます。クリックするとワークフローファイルが開き、サイドバーが Workflow / skill タブに切り替わるので、そのまま編集・再実行できます。さらに **AI でワークフローを修正** と **実行履歴を参照** を組み合わせれば、どのステップがどんな入力で失敗したかを AI に直接確認させて、修正を指示できます。
