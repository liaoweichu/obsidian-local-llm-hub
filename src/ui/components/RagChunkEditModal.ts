...
export class RagChunkEditModal extends Modal {
  private result: RagSearchResult;
  ...
  private async llmChat(userContent: string, systemPrompt: string): Promise<string> {
    ...
    for await (const chunk of localLlmChatStream(
      this.llmConfig, messages, systemPrompt, this.abortController?.signal,
    )) {
  ...
  private async collectChunks(direction: "prev" | "next", count: number): Promise<RagSearchResult[]> {
    const store = getRagStore();
  ...
  private applyCollectedChunks(prevChunks: RagSearchResult[], nextChunks: RagSearchResult[]): void {
    if (!this.textarea) return;
  ...
  private async refineWithAI(btn: HTMLButtonElement): Promise<void> {
    ...
    try {
      ...
      for await (const chunk of localLlmChatStream(
        this.llmConfig, messages, refineSystemPrompt, this.abortController.signal,
      )) {
        if (chunk.type === "text" && chunk.content) {
